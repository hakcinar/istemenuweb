...
const Page = ({ params: { foodId, categoryId, restaurantNo } }) => {
  ...
  const basketHandler = () => {
    ...
    const currentBasket = basket.length > 0 ? basket : JSON.parse(localStorage.getItem('basket')) || [];
    // Check if the food already exists in the basket
    const existingFoodIndex = currentBasket.findIndex(item => item.id === food.id);
    let newBasket;
        ...
        <div key={foodId}>
          ...
          <div className="py-4 px-4">
            <div className="flex justify-between items-center">
              ...
              <span className="text-yellow text-xl mr-6 font-bold">
                {food.cost} ₺
              </span>
            </div>
            ...
            <div>
              <h5 className="text-yellow mb-2 font-bold hidden">Porsiyon Seçiniz</h5>
              ...
              <textarea
                ...
                value={note}
                className="pt-3 pb-2 px-4 w-full  outline-none border-none rounded-xl placeholder:text-m font-bold bg-white flex items-center justify-center  overflow-hidden mb-4"
                placeholder="Eklemek veya Çıkarmak İstediğiniz Malzemeyi Yazınız"
              />
              <button
                onClick={basketHandler}
                className="text-black font-bold flex justify-between items-center text-bold bg-yellow border-2 font-bold rounded-xl text-xl border-yellow outline-none px-4 py-2 w-full"
              >
                Sepete Ekle
                {basketIcon}
              </button>
